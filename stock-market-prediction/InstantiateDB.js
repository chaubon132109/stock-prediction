...
const uri = `mongodb+srv://${configDB.user}:${configDB.password}@${configDB.server}${configDB.database}?retryWrites=true&w=majority`;

/**
 * Update MongoDB with incoming logs.
 * @param {JSON} message - Incoming messages to be updated to DB.
 * @param {String} collectionInterface - Connect to specific collection of the DB.
 * @param {Boolean} stream - If incoming messages are from a kafka stream or not.
 **/
async function updateMongoDB(
  message,
  collectionInterface = configDB.collectionStream,
  stream = true
) {
  if (stream == true) {
    ...
    console.log("Ml_Prediction ", message);
    logs_ = message;
  }
  let client = await MongoClient(uri, {
...
async function getDBInfo() {
  ...
  client.connect((err) => {
    ...
    const collection = client
      .db(configDB.database)
      .collection(configDB.collectionStream);
    collection
      .find(
        {},
        {
