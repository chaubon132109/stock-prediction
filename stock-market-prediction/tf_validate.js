 ...
 **/
async function val() {
  let client = new db.MongoClient(db.uri, {
  ...
  client.connect((err) => {
    ...
    collection
      ...
      .toArray(async function (err, result) {
        ...
        var test_data = result.slice(
          parseInt((train.trainSize / 100) * result.length),
          result.length
        );
        var test_data_inp = result.slice(
          result.length - test_data.length - timesteps,
          result.length
        );
